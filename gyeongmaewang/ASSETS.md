...
| 이도현 | ✅ 반영 완료 | ✅ 반영 완료 | ✅ 반영 완료 | ✅ 반영 완료 |
| 윤미정 | ✅ 반영 완료(v104) | ✅ 반영 완료 | ✅ 반영 완료(v104) | ✅ 반영 완료(v104) |
| 박재훈 | ✅ 반영 완료 | ✅ 반영 완료 | ✅ 반영 완료 | ✅ 반영 완료 |
| 최은경 | ✅ 반영 완료 | ✅ 반영 완료 | ✅ 반영 완료 | ✅ 반영 완료 |
| 김태식 | ✅ 반영 완료 | ✅ 반영 완료 | ✅ 반영 완료 | ✅ 반영 완료 |
...
| 캐릭터 | 상태 |
|---|---|
| 한서윤 「반지하 방의 첫 밤」 | ✅ 반영 완료 |
| 이도현 「퇴근 후 두 번째 출근」 | ✅ 반영 완료 |
| 윤미정 「단골이 된 중개사」 | ✅ 반영 완료 |
| 박재훈 「벽을 두드리던 손」 | ✅ 반영 완료 |
| 최은경 「엑셀 시트의 마지막 줄」 | ✅ 반영 완료 |
| 김태식 「태식의 오래된 수첩」 | ✅ 반영 완료 |

## 인생의 갈림길 그림 6종

| 캐릭터 | 상태 |
|---|---|
| 한서윤 · 이도현 · 윤미정 · 박재훈 · 최은경 | ✅ 반영 완료 |
| 김태식 「혼자 다니지 마세요」 | ✅ 반영 완료 |
...
| 이번 판 돌아보기 | 코르크 보드 |
| 1년 결산 패널 | 결산 날 새벽 책상 |
| CASE 002 잔금 갈림길 | 밤 책상 · 통장 |
| CASE 002 매도 소식 | 같은 건물 급매 / 덮은 누수 재발 |
| CASE 002 결과 | 제대로 고친 방 · 청구서 · 업자에게 넘김 · 잔금 포기 · 누수 알리고 판매 · 계약 성사 |
| 기록 — 자산 등급 물건 | 구축 빌라 · 신축 빌라 · 다가구 · 상가주택 · 꼬마빌딩 (잠긴 등급은 흑백) |
| 조사 화면 — 시기 그림 | 이사철 · 장마철 · 휴가철 · 추석 · 설 · 연말연초 · 겨울 비수기 (판 날짜가 그 시기면 모바일은 날씨 그림 자리, PC는 시기 영향 펼침 맨 위) |
| 홈 — 연습실 타일 | 명도왕(도어록·열쇠) · 협상(머그컵·말풍선) · 매도(가격표·계산기) · 시세(지도·돋보기) |
| 조사 파일 — 벽 뒤 누수 카드 | 벽지 걷어낸 누수 |
| 홈 — 큰 타일 | 스토리(일기장·사진) · 도감(실루엣 카드 앨범) · 기록(저금통·막대 블록) |
| 그때 그 물건 소식 | 아까운 결과 → 불 켜진 창을 올려다보는 뒷모습 · 피한 결과 → 방수포 덮인 공사 중 빌라 |

CASE 002 그림 전부 반영 — C7 매수자 대출 → 매도 중 계약 파기 소식. (C1 외관 → 게시판 2번 물건 사진, C2 누수 발견 → 수리 단계 누수 카드 , C3 도배 → "도배로 덮기" 고른 뒤 호가 화면 — 반영 완료)

(후일담: 김태식 그림을 서재·첫 장·연필 버전으로 교체 — 엔딩 NORMAL과 겹치지 않게.)

## 조사에서 만난 사람 (v94)

조사 결과에 그 사람의 말이 나오면 조사 무대에 얼굴·이름·말이 뜬다(`npcface.js`). 그림 없는 사람은 예전처럼 글로만.

| 인물 | 상태 |
|---|---|
| 김사장 · 박 중개사 · 아래층 아주머니 · 은행 대출상담 | ✅ 반영 |
| 최만식 할아버지 · 정은주 씨(점유자 그림 재사용) | ✅ 반영 |
| 전기기사 박기사 (v95) | ✅ 반영 — CASE 001 "계량기·차단기 흔적 확인"에서 사진 보고 한마디, 하자 단계에 직접 와서 재 봄(조사로 알았는지에 따라 대사가 갈림). 인생 모드에서 단골 최 기사가 와 준 판에는 안 뜬다 |
| 인테리어 박실장 (v96) | ✅ 반영 — CASE 002 하자 단계, 벽지 뒤 물길을 본 순간. 누수를 미리 알았으면 팔짱 낀 차분한 얼굴, 몰랐으면 손을 들어 "더 뜯지 마세요!" |
| 관리인 할아버지 · 관리실 직원 · 중개사 A · 중개사 B (v98) | ✅ 반영 — 관리실 방문·전화, 중개사 3곳 전화·방문 조사. 여러 명이 말하면 조사할 때마다 돌아가며 한 명씩 |
| 중개사 C (v99) | ✅ 반영 — 3곳 조사에서 A·B·C가 번갈아 한 명씩 |
