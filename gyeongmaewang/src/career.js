...
homeHTML = function(){
  ...
  return `<div class="vn-title hub-hero kc-title">${vnBgHTML("bg_villa_night")}<div><h3 class="kc-logo">🏆 경 매 왕</h3><p class="kc-tag">싸게 사는 것만으로는 돈을 벌 수 없습니다.</p>
   ...
   <div class="hub-grid sm">
     <button type="button" class="panel hub-tile hub-prac" data-atab="game"><span class="hub-thumb" style="background-image:url('${window.GMW_STANDALONE ? "assets/2379ac4f4b7fcb77e7bbdee9e268a508.webp" : "/_blob/2379ac4f4b7fcb77e7bbdee9e268a508"}')" aria-hidden="true"></span><span class="hub-ic">🎮</span><b>명도왕</b><small>점유자 한 명과 주 단위 명도</small></button>
     <button type="button" class="panel hub-tile hub-prac" data-atab="chat"><span class="hub-thumb" style="background-image:url('${window.GMW_STANDALONE ? "assets/3568d02038b54754cb273a5326355d70.webp" : "/_blob/3568d02038b54754cb273a5326355d70"}')" aria-hidden="true"></span><span class="hub-ic">💬</span><b>협상 연습</b><small>AI 점유자·중개사와 대화</small></button>
     <button type="button" class="panel hub-tile hub-prac" data-atab="sell"><span class="hub-thumb" style="background-image:url('${window.GMW_STANDALONE ? "assets/65e4bad7bbbfe85ae651453234ec16b7.webp" : "/_blob/65e4bad7bbbfe85ae651453234ec16b7"}')" aria-hidden="true"></span><span class="hub-ic">🏷️</span><b>매도 연습</b><small>호가 정하기</small></button>
     <button type="button" class="panel hub-tile hub-prac" data-atab="guess"><span class="hub-thumb" style="background-image:url('${window.GMW_STANDALONE ? "assets/41682866d171ba9b5d08e7fb880eb4cc.webp" : "/_blob/41682866d171ba9b5d08e7fb880eb4cc"}')" aria-hidden="true"></span><span class="hub-ic">🎯</span><b>시세 맞히기</b><small>실제 과거 거래로</small></button></div>`;
};
function recHTML(){
  const c = kcRec(), p = hubRec(), wk = kcWeek(), W = c.weekly[wk];
  const tier = KC_TIERS.map(([need, t], ti) => `<li class="${c.cash >= need ? "on" : ""}">${typeof KC_TIER_ART !== "undefined" && KC_TIER_ART[ti] ? `<img class="kc-tier-art" src="${window.GMW_STANDALONE ? "assets/" + KC_TIER_ART[ti] + ".webp" : "/_blob/" + KC_TIER_ART[ti]}" alt="" loading="lazy">` : ""}<span>${c.cash >= need ? "🔓" : "🔒"} ${esc(t)}</span><small>자산 ${kMan(need)}${c.cash >= need ? "" : ` · ${kMan(need - c.cash)} 더`}</small></li>`).join("");
