...
function k2HTML(){
  ...
  if(K.step === "list"){
    return kStage(K.k2.cond === "fix" ? (artUrl("bg_room_empty") ? "bg_room_empty" : "bg_room_clean") : "bg_room_messy", "narr", null, "이제 얼마에 내놓을까. 김사장과 박 중개사의 말이 다르다.") + kHud() + `
  ...
  if(K.step === "sell"){
    ...
    return kStage("bg_realtor", "narr", null, line) + kHud() + `
     <div class="panel k-card"><div class="k-grid"><span>현재 호가</span><b>${kMan(S.list)}</b><span>보유 비용</span><b>매주 약 ${kMan(KP2.dailyHold * 7)}</b>${S.weeks<=3?`<span>경쟁</span><b>같은 건물 3층 1억 6,200 급매</b>`:""}</div>${S.note?`<div class="note" style="margin-top:6px">${esc(S.note)}</div>`:""}${typeof gmwSceneArt!=="function" ? "" : /또 물이 샌다/.test(S.note||"") ? gmwSceneArt("k2recur") : /급매가 .*팔렸다/.test(S.note||"") ? gmwSceneArt("k2rival") : /대출이 생각보다 안 나와서/.test(S.note||"") ? gmwSceneArt("k2loan") : ""}</div>
