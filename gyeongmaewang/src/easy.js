...
function ezTut(){   // 한서윤의 첫 물건 = 튜토리얼(초보 모드 자동 + 단계마다 선배 설명)
  if(typeof K === "undefined" || !K || K.mode === "weekly") return false;
  if(K.tut === undefined){ K.tut = !!(typeof lfIs === "function" && lfIs("seoyun") && !(+kcRec().cases) && (!navigator.webdriver || window.EZ_TUT_FORCE)); }
  return K.tut;
}
function ezOn(){ return !!kcRec().ez || ezTut(); }
...
/* ---------- 🎓 튜토리얼 — 단계에 처음 들어갈 때 선배가 한 번씩 ---------- */
const EZ_COACH = {
  intro:["사건 소개", "이번이 첫 물건이야. 오늘은 내가 옆에서 단계마다 짧게 설명해 줄게.", "위에 초록 줄(🧭 지금 할 일)이 지금 뭘 하는지 알려 주고, 노란 테두리 ⭐가 눌러 볼 버튼이야. 파란 점선 단어는 누르면 뜻이 나와."],
  research:["조사", "경매는 '싸게 사는 게임'이 아니라 '모르는 비용을 찾는 게임'이야.", "사건 파일(서류)과 전화·현장 조사로 숨은 위험 — 떠안을 돈, 수리비, 점유자 사정 — 을 찾아. 시간이 정해져 있어서 다 못 해. ⭐가 정보가 많이 나오는 순서야."],
  bid:["입찰", "입찰가는 '팔 가격 − 들어갈 돈 전부 − 남기고 싶은 돈'으로 정해.", "들어갈 돈 = 취득세·수리비·이사비·이자·중개비. 입찰표 옆 '예상 세전 수익'이 플러스인지 꼭 봐. 떨어져도 보증금은 돌려받으니까 무리해서 올리지 마."],
  won:["낙찰", "낙찰! 잔금을 내는 날 이 집 주인이 돼.", "근데 아직 안에 사람이 살고 있어. 다음 단계가 명도야."],
  lost:["패찰", "졌어도 괜찮아. 보증금은 그대로 돌려받아.", "무리해서 이기는 것보다 원칙대로 지는 게 나아. 다음 물건에서 또 기회가 와."],
  move:["명도", "명도는 '내쫓기'가 아니라 '같이 나갈 날 정하기'야.", "먼저 사정을 들어 봐. 왜 못 나가는지 알아야 풀 수 있어. 마음이 열리면(협조도 45쯤) 날짜를 정하자고 해. 겁주기는 법적 절차가 아니라 역효과고, 대화가 안 되면 인도명령이라는 법원 절차가 있어."],
  defect:["수리", "집을 열어 보면 꼭 고칠 게 나와.", "조사 때 미리 찾은 건 이미 계산에 넣어 둔 거라 덜 아파. 수리는 '팔 때 값을 올려 주는 만큼만' — 처음엔 가성비나 일부 수리가 무난해."],
  list:["호가", "호가는 '이 가격에 팔게요' 하고 내거는 값이야.", "너무 높으면 몇 달씩 안 팔리고 그동안 이자가 계속 나가. 처음엔 시세 근처로 내놓자."],
  sell:["매도", "사겠다는 사람이 오면 금액부터 봐.", "호가랑 거의 같으면 받아도 되고, 많이 낮으면 중간값으로 역제안하거나 기다려. 매수자마다 사정이 달라서, 사정을 들어 보면 흥정이 잘 통하기도 해."],
  result:["결과", "첫 사건 끝! 수고했어.", "결과 화면에서 '놓친 위험'이 뭐였는지 봐 둬. 다음 물건부터는 내 설명 없이 해 보자 — 막히면 ☰ 메뉴에서 초보 모드를 켜면 한 줄 안내와 ⭐는 계속 나와."]};
function ezCoachKey(){
  if(!K) return null;
  if(K.intro) return "intro";
  if(K.revealing) return null;
  if(K.step === "brief"){
    if(K.sealed) return "bid";
    const hid = (KP.hidden || []).length, found = (KP.hidden || []).filter(h => K.found[h.id]).length, acts = (KP.actions || []).filter(a => typeof krCan === "function" && krCan(a));
    return !acts.length || found >= hid ? "bid" : "research";
  }
  return EZ_COACH[K.step] ? K.step : null;
}
function ezCoach(){
  if(!ezTut() || document.getElementById("ezCoach") || document.querySelector("#mtTale, #hxRoot, .hx-talk, #ezAsk")) return;
  const k = ezCoachKey(); K.tutSeen = K.tutSeen || {}; if(!k || K.tutSeen[k]) return;
  K.tutSeen[k] = true; const c = EZ_COACH[k], order = ["intro","research","bid","won","move","defect","list","sell","result"], n = order.indexOf(k);
  const d = document.createElement("div"); d.id = "ezCoach"; d.className = "hx-talk ez-coach";
  d.innerHTML = `<div class="hx-talk-box"><div class="ez-coach-h"><b>🎓 선배의 한마디 — ${c[0]}</b>${n >= 0 ? `<small>${n + 1} / ${order.length}</small>` : ""}</div><p class="ez-coach-big">${c[1]}</p><p>${c[2]}</p><div class="hx-talk-acts"><button type="button" class="btn primary" data-ezcoach>알겠어요 👍</button></div></div>`;
  document.body.appendChild(d);
}
/* ---------- 처음 한 번 묻기 + 메뉴 토글 ---------- */
function ezAsk(){
...
function ezMenu(){
  ...
  b.textContent = ezTut() ? "🎓 튜토리얼 그만 보기" : ezOn() ? "🐣 초보 모드 끄기" : "🐣 초보 모드 켜기";
}
document.addEventListener("click", e => {
  let b;
  if(e.target.closest && e.target.closest("[data-ezcoach]")){ const d = document.getElementById("ezCoach"); if(d) d.remove(); try{ ezPaint(); }catch(_){} return; }
  ...
  if((b = e.target.closest && e.target.closest("[data-eztoggle]"))){ e.preventDefault(); e.stopPropagation(); if(ezTut()) K.tut = false; else ezSet(!ezOn()); const m = document.querySelector(".kfs-menu"); if(m) m.hidden = true; try{ renderArena(); }catch(_){} return; }
...
if(typeof renderArena === "function"){
  ...
  renderArena = function(){
    ...
    try{
      ezMenu();
      if(K && arenaTab === "king" && !kcRec().ezAsked && !navigator.webdriver && !ezTut()) ezAsk();   // 튜토리얼 중엔 묻지 않는다   // 자동 테스트(브라우저 자동화)에서는 묻지 않는다
      ezPaint(); setTimeout(() => { try{ ezPaint(); ezMenu(); if(K && arenaTab === "king") ezCoach(); }catch(_){} }, 80);
    }catch(e){}
    return r;
  };
}
