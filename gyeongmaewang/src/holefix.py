...
import glob, os
for n in [os.path.basename(f)[:-5] for f in sorted(glob.glob(f'{od}/*.webp'))]:
...
ims=[Image.open(f'{od}/{n}.webp').convert('RGBA') for n in [os.path.basename(f)[:-5] for f in sorted(glob.glob(f'{od}/*.webp'))]]
