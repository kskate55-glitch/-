...
function rbApply(){
  ...
  // 최저가에 딱 맞춰 쓰는 사람은 드물다 — 몇십만~몇백만 원은 얹는다(1.005~1.06배)
  K.rivals = K.rivals.map(v => { const [a, b] = rbStyle(v.t || ""); const lo = Math.max(1.005, rate * a / m), hi = Math.max(lo + 0.05, rate * b / m); return Object.assign({}, v, {lo, hi, rb:1}); });
}
