...
for(const [w,h] of [[1280,800],[390,844]]){
 ...
 for(const r of ['trade','docs','call1']){ const el=await p.$(`[data-kres="${r}"]:not([disabled])`); if(el){ await el.click(); await p.waitForTimeout(120);} }
 ...
 for(const r of ['ext','meter','neigh']){ const el=await p.$(`[data-kres="${r}"]:not([disabled])`); if(el){ await el.click(); await p.waitForTimeout(120);} }
