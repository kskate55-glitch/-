...
const SEL='[data-lfspot],[data-lfdo]:not([disabled]),[data-lfbuy]:not([disabled]),[data-lfnext],[data-bdplay]:not([disabled]),[data-bdread],[data-bddecoy]:not([disabled]),[data-bdwait]:not([disabled]),[data-bdwatch],[data-bdnext]:not([disabled]),[data-kres]:not([disabled]),[data-kcseal],[data-kbid],[data-kintro],[data-kgo],[data-kmove],[data-koffer],[data-kflip],[data-krep],[data-klist],[data-ksale],[data-k2],.lf-after [data-atab="life"]';
for(const [w,h,ch,seed] of [[1280,800,'taesik',3],[390,844,'seoyun',4],[1280,800,'eunkyung',5],[390,844,'mijeong',6]]){
 ...
 for(let i=0;i<700;i++){
  const r=await p.evaluate((SEL)=>{
    ...
    if(!btns.length) return {bad, stuck:arenaTab+'|'+(window.K?K.step:'')+'|'+t.slice(0,80)+'|SPOT='+LF_SPOT+'|intro='+(lfRec()&&lfRec().intro)+'|root='+root.innerHTML.slice(0,300)+'|main='+document.getElementById('main').innerHTML.slice(0,200)};
    const bid=document.getElementById('kBid'); if(bid) bid.value=String(Math.round((KP.minBid+Math.random()*KP.minBid*0.3)/10)*10);
    const el=btns[Math.floor(Math.random()*btns.length)]; const a=[...el.attributes].map(a=>a.name).find(n=>n.startsWith('data-'));
