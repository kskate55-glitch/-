...
for(const [w,h,ch,seed] of [[1280,800,'taesik',3],[390,844,'seoyun',4],[1280,800,'eunkyung',5],[390,844,'mijeong',6]]){
 ...
 for(let i=0;i<1200;i++){
  const r=await p.evaluate((SEL)=>{
    ...
    if(!btns.length) return {bad, stuck:[...root.querySelectorAll('button')].map(b=>b.outerHTML.slice(0,90)).join(' || ').slice(0,1500)+' @@ '+arenaTab+'|'+(typeof K!=='undefined'&&K?K.step:'')+'|'+t.slice(0,80)};
