...
for(const [w,h] of [[1280,800],[390,844]]){ const p=await b.newPage({viewport:{width:w,height:h}}); p.on('pageerror',e=>errs.push(e.message));
 ...
 const drop = await p.$('[data-bddrop]'); if(drop){ const n0 = await p.evaluate(()=>document.querySelectorAll('[data-bddrop]').length); await drop.click(); await p.waitForTimeout(450); await p.click('.sc-armed'); await p.waitForTimeout(300); ok(await p.evaluate(n0=>document.querySelectorAll('[data-bddrop]').length<n0, n0), w+' 게시판 포기도 두 번 누르면 됨'); }
