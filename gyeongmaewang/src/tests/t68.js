// 박재훈 그림 — 선택 카드 얼굴 · 포커스 세로 일러스트 · 거점 방/전신 · 대화 뒷모습 · 표정 · 오프닝 현장 컷
...
for(const [w,h] of [[1280,800],[390,844]]){ const p=await b.newPage({viewport:{width:w,height:h}}); p.on('pageerror',e=>errs.push(e.message)); p.on('response',r=>{ if(r.status()>=400) bad.push(r.status()+' '+r.url()); });
 ...
 const A = await p.evaluate(()=>LF_CHAR_ART.jaehoon);
 await p.evaluate(()=>{ localStorage.clear(); cpUnlockAll(); const c=kcRec(); delete c.life; K=null; arenaTab='life'; LF_PICK='jaehoon'; renderArena(); }); await p.waitForTimeout(700);
 ...
 ok(sel.includes(A.face.normal), w+' 선택 카드 얼굴'); ok(sel.includes(A.select), w+' 포커스 세로 일러스트'); await p.screenshot({path:`jh_sel_${w}.png`});
 await p.evaluate(()=>{ lfNew('jaehoon'); lfRec().intro=false; LF_SPOT='laptop'; renderArena(); }); await p.waitForTimeout(600);
 ...
 ok(base.me.includes(A.front), w+' 거점에 재훈 전신'); ok(base.html.includes(A.room), w+' 불광동 작업방 배경'); await p.screenshot({path:`jh_base_${w}.png`});
 ...
 await p.screenshot({path:`jh_won_${w}.png`});
 await p.evaluate(()=>{ K=null; gxOpStart('jaehoon'); }); let hit=false;
 for(let i=0;i<30 && !hit;i++){ await p.waitForTimeout(500); hit = await p.evaluate(u=>[...document.querySelectorAll('.gx-bgimg')].some(e=>e.style.backgroundImage.includes(u)), A.site); if(i===4) await p.keyboard.press('Space'); }
 ok(hit, w+' 오프닝 첫 장면 = 수리 현장 컷');
 await p.screenshot({path:`jh_op_${w}.png`});
