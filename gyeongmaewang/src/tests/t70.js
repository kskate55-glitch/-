// 박태식 그림 — 선택 카드 얼굴 · 포커스 세로 일러스트 · 거점 방/전신 · 대화 뒷모습 · 표정 · 오프닝 현장 컷
...
for(const [w,h] of [[1280,800],[390,844]]){ const p=await b.newPage({viewport:{width:w,height:h}}); p.on('pageerror',e=>errs.push(e.message)); p.on('response',r=>{ if(r.status()>=400) bad.push(r.status()+' '+r.url()); });
 ...
 const A = await p.evaluate(()=>LF_CHAR_ART.taesik);
 await p.evaluate(()=>{ localStorage.clear(); cpUnlockAll(); const c=kcRec(); delete c.life; K=null; arenaTab='life'; LF_PICK='taesik'; renderArena(); }); await p.waitForTimeout(700);
 ...
 ok(sel.includes(A.face.normal), w+' 선택 카드 얼굴'); ok(sel.includes(A.select), w+' 포커스 세로 일러스트'); await p.screenshot({path:`tk_sel_${w}.png`});
 await p.evaluate(()=>{ lfNew('taesik'); lfRec().intro=false; LF_SPOT='laptop'; renderArena(); }); await p.waitForTimeout(600);
 ...
 ok(base.me.includes(A.front), w+' 거점에 태식 전신'); ok(base.html.includes(A.room), w+' 목동 거실 배경'); await p.screenshot({path:`tk_base_${w}.png`});
 ...
 await p.screenshot({path:`tk_won_${w}.png`});
 await p.evaluate(()=>{ K=null; gxOpStart('taesik'); }); let hit=false;
 ...
 await p.screenshot({path:`tk_op_${w}.png`});
