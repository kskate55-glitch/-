...
const MODES={novice:'합리적인 입문자', skilled:'목적 있는 숙련자', full:'조사 완주형', reckless:'무리한 플레이'};
const rows=[];
for(const id of ids){ for(const mode of Object.keys(MODES)){
  const r=await p.evaluate(([id,mode])=>{
    ...
    if(mode==='novice') ['docs','trade','court'].forEach(x=>kResearch(x));
    ...
    else { const M=qaBidModel(); bid=Math.round((M.z.bal - (mode==='novice'?0:defKnown))/10)*10;   // 입찰표가 안내하는 '균형 구간' 끝(알아낸 인수·다툼 비용은 이미 빠져 있다) — 알아낸 하자 비용만 더 뺀다
      if(bid<KP.minBid){ if(mode==='novice') bid=KP.minBid; else return {pass:true, profit:0}; } }
    ...
    return {profit:Math.round(K.final.profit), days:K.day, cashIn:Math.round(cashIn), cost:JSON.stringify(K.cost), sale:K.sale.price, bidv:K.bid, trueMid:KP.trueMid};
  },[id,mode]);
...
for(const [mode,name] of Object.entries(MODES)){
  ...
  md+=`| ${name} | ${done.length} | ${P.filter(v=>v>0).length} | ${P.filter(v=>v<=0).length} | ${R.filter(r=>r.pass||r.lost).length} | ${man(tot)} | ${man(tot/Math.max(1,done.length))} | ${man(P[0]||0)} | ${man(lowAvg||0)} | ${Math.round(done.reduce((s,r)=>s+r.days,0)/Math.max(1,done.length))}일 |\n`;
}
md+='\n## 물건별\n\n| 물건 | 입문자 | 숙련자 | 완주형 | 무리 |\n|---|---|---|---|---|\n';
