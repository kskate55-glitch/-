...
for(const mode of Object.keys(BOTS)){
  await p.evaluate((start)=>{ localStorage.clear(); const c=kcRec(); c.fr={full:true}; c.cash=start; c.credit=[]; delete c.life; },START);
  const log=[];
  for(const id of ids){
    const r=await p.evaluate(([id,mode])=>{
      ...
      let cr=0; if(crGap()>0){ if(mode==='cash') return {id, skip:'보증금도 없음', cash:cash0};
        const o=CR_OFFERS.slice().sort((a,b)=>a.rate-b.rate).find(o=>crLeft(o)>=crGap()); if(!o || !crTake(o.id)) return {id, skip:'보증금도 없음', cash:cash0}; cr=crOwed(); }
      ...
      if(bid<KP.minBid){ crRepay(7); return {id, skip:'포기', cash:kcRec().cash}; }
      if(mode==='cash' && bid*(1+kAcqRate(bid))+300 > cash0) return {id, skip:'현금 부족', cash:cash0};
      let tries=0; while(tries++<(mode==='lev'?4:3)){ kBid(bid); K.revealing=false; K.sealed=false; if(K.step==='won') break; if(mode!=='lev'){ crRepay(7); return {id, skip:'패찰', cash:kcRec().cash}; } bid=Math.round(bid*1.06/10)*10; K.step='brief'; }
      if(K.step!=='won'){ crRepay(7); return {id, skip:'패찰', cash:kcRec().cash}; }
      // 대출
      ...
      crRepay(0); return {id, profit:Math.round(K.final.profit), cash:kcRec().cash, credit:cr, owed:crOwed(), debt, rate, need:Math.round(need), selfIn:Math.round(need-debt), days:K.day, hold:Math.round(K.cost.hold)};
    },[id,mode]);
    log.push(r);
  }
  out[mode]=log;
