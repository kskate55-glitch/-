...
for(const [w,h] of [[1280,800],[390,844]]){
 ...
 for(const id of ['seoyun','dohyun','mijeong','jaehoon']) for(const hi of [true,false]){
  ...
  const want = id!=='jaehoon';
  ok((want ? r.pose>0 && r.playerHidden : r.pose===0 && !r.playerHidden) && r.say && r.headOk && r.sayOk, `${w} ${id} ${r.step}: ${JSON.stringify(r)}`);
  if(id!=='jaehoon'){ await p.waitForTimeout(2500); await p.screenshot({path:`t_pose_${id}_${r.step}_${w}.png`}); }
 }
 await p.close();}
