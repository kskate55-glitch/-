...
for(const [w,h] of [[1280,800],[390,844]]){
 ...
 for(const [ch,hi] of [['dohyun',false],['dohyun',true],[null,false],['taesik',2],['seoyun',3]]){
  const r=await p.evaluate(async ([ch,hi])=>{ localStorage.clear(); try{cpUnlockAll()}catch(e){} if(ch){ lfNew(ch); lfRec().intro=false; } arenaTab='king'; KC_MODE='career'; K_PROP_NEXT='k1'; KC_INTRO=false; kStart(5); K.intro=false; K.timeLeft=9999;
    K.rivals= hi===3 ? [{t:"감정가 맹신러",lo:1.10,hi:1.16},{t:"전문 투자자",lo:1.06,hi:1.12}] : hi===2 ? [{t:"이사철 실수요자",lo:1.10,hi:1.16},{t:"겉 마진 보고 온 투자자",lo:1.06,hi:1.12},{t:"감정가 맹신러",lo:1.0,hi:1.01}] : [{t:"지역 실수요자",lo:1.10,hi:1.16},{t:"전문 투자자",lo:1.06,hi:1.12},{t:"감정가 맹신러",lo:1.0,hi:1.01}];
  ...
  const nm = hi===3 ? '감정가 맹신러' : hi===2 ? '이사철 실수요자' : '지역 실수요자'; const exp = new RegExp((r.step==='lost'?'1등':'2등')+' · '+nm);
  ok(r.faces===r.rows-1 && exp.test(r.tag||'') && r.sprite>0 && r.headOk && !r.overlap && r.inView, `${w} ${ch||'기본'} ${r.step}: ${JSON.stringify(r)}`);
