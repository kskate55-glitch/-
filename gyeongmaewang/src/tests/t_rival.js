...
for(const [w,h] of [[1280,800],[390,844]]){
 ...
 for(const [ch,hi] of [['dohyun',false],['dohyun',true],[null,false]]){
  const r=await p.evaluate(async ([ch,hi])=>{ localStorage.clear(); try{cpUnlockAll()}catch(e){} if(ch){ lfNew(ch); lfRec().intro=false; } arenaTab='king'; KC_MODE='career'; K_PROP_NEXT='k1'; KC_INTRO=false; kStart(5); K.intro=false; K.timeLeft=9999;
    K.rivals=[{t:"지역 실수요자",lo:1.10,hi:1.16},{t:"전문 투자자",lo:1.06,hi:1.12},{t:"감정가 맹신러",lo:1.0,hi:1.01}];
    kBid(hi ? Math.round(KP.minBid*1.8/10)*10 : KP.minBid); K.revealing=false; renderArena(); await new Promise(r=>setTimeout(r,600));
  ...
  const exp = r.step==='lost' ? /1등 · 지역 실수요자/ : /2등 · 지역 실수요자/;
