...
r=await p.evaluate((F)=>{ localStorage.clear(); const c=kcRec(); delete c.life; c.fr={full:true}; arenaTab='king'; KC_MODE='career'; K_PROP_NEXT='f41'; kStart(4); K.intro=false;
  const before=vnBgPick('bg_front_door'), occ0=artNpc(KP.occ.pid,'normal');
  artRec()['bg_case_f41_door']=F; artRec()['npc_occ_f41_normal']=F;
  ...
  delete artRec()['bg_case_f41_door']; delete artRec()['npc_occ_f41_normal'];
  return {before, after, other, occ0, occ1, has:stage.includes(F)}; },FAKE);
ok(r.before!=='bg_case_f41_door' && r.after==='bg_case_f41_door', '물건 전용 현관 그림을 올리면 f41 명도 장면 배경이 바뀜');
ok(r.other!=='bg_case_f41_door', '다른 물건(f21)은 그대로');
ok(r.occ1 && r.occ1.includes(FAKE) && r.has, 'f41 점유자 그림(보통만 올려도 화난 표정 자리에 대신) 무대에 뜸');
