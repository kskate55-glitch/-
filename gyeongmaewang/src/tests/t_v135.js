...
r=await p.evaluate((F)=>{ localStorage.clear(); const c=kcRec(); delete c.life; c.fr={full:true}; arenaTab='king'; KC_MODE='career'; K_PROP_NEXT='f22'; kStart(4); K.intro=false;
  ...
  artRec()['bg_case_f22_door']=F; artRec()['npc_occ_f22_normal']=F;
  ...
  delete artRec()['bg_case_f22_door']; delete artRec()['npc_occ_f22_normal'];
  return {before, after, other, occ0, occ1, has:stage.includes(F)}; },FAKE);
ok(r.before!=='bg_case_f22_door' && r.after==='bg_case_f22_door', '물건 전용 현관 그림을 올리면 f22 명도 장면 배경이 바뀜');
ok(r.other!=='bg_case_f22_door', '다른 물건(f21)은 그대로');
ok(r.occ1 && r.occ1.includes(FAKE) && r.has, 'f22 점유자 그림(보통만 올려도 화난 표정 자리에 대신) 무대에 뜸');
