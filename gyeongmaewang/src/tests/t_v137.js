...
// 서윤 오프닝 전용 그림 3장
const o=await p.evaluate(async()=>{ const ks=['op_seoyun_1','op_seoyun_1_2','op_seoyun_2_3']; const us=ks.map(k=>artUrl(k));
  ...
  return {all:loads.every(Boolean), distinct:new Set(us).size, other:!!artRec()['op_dohyun_1'] || (ART_DEFAULT['op_dohyun_1']||null)}; });
ok(o.all && o.distinct===3, '서윤 오프닝 3장(방·창밖·생각구름)이 로드됨');
