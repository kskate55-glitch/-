...
const g=await p.evaluate(async()=>{ localStorage.clear(); const c=kcRec(); delete c.life; c.fr={full:true}; page='arena'; arenaTab='king'; KC_MODE='career'; K_PROP_NEXT='f11'; KC_INTRO=false; kStart(5); K.intro=false;
  ...
  K_PROP_NEXT='f43'; kStart(5); K.intro=false; const other=vnBgPick('bg_front_door');
...
ok(g.other==='bg_front_door', '배경이 아직 없는 물건(f43)은 원래 공용 배경 그대로');
...
ok(g10.f41==='bg_case_f41_ext,bg_case_f41_door,bg_room_empty' && g10.f42==='bg_case_f42_ext,bg_case_f42_door,bg_room_empty', 'f41·f42 외관·현관 전용(실내는 아직 공용)');
