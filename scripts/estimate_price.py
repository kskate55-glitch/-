...
#
# ⛔ **그리고 실측에서 되돌렸다(72-14절·72-15절).** 위 분석은 전부 합성
#    데이터였고, 실제 백테스트 **짝지은 72건**(경기 59 + 서울 13, 버전
#    3a77093 대 f2d709a/ca3f28f)에서 **어느 지표로도 효과가 없었다**:
#
#    | 지표 | 거리 35%(예전) | 거리 65% | |
#    |---|---|---|---|
#    | 평균 \|오차\| | 11.16% | 10.94% | −0.22%p, 95%[−0.79,+0.40] ➖ |
#    | 좋아짐/나빠짐 | — | **29 대 29** | 부호검정 **p = 1.000** |
#    | **25% 넘게 틀림** | **7건** | **7건** | ← 이 변경이 노린 바로 그 지표 |
#    | 90분위(꼬리) | 24.70% | 24.82% | +0.12%p ➖ |
#
#    ⚠️ **"최악의 손해"로 골랐으면서 판정은 평균으로 하려 했다**(72-13절이
#    정한 기준이 그랬다). 꼬리를 따로 재고 나서야 제대로 본 것인데, **꼬리도
#    똑같이 null**이었다 — 평균을 봤든 꼬리를 봤든 결론은 같다.
#
#    되돌리는 근거는 "나빠져서"가 아니라 **"주장한 이득이 확인되지 않아서"**다
#    (48-2절 SALE_CALIBRATION_FACTOR 0.97과 같은 처리). 아래 값은 48~70절의
#    모든 보정(예측구간 폭 ±20/30/35 · 경고 임계값 · 64절 1층 계수)이
#    측정된 바로 그 배분이다 — 이득이 없는 이탈을 안고 갈 이유가 없다.
#
#    ⚠️ 다만 **서울 13건만 보면 −1.29%p [−2.91, +0.05]로 좋은 쪽에 붙어
#    있었다**(경기 59건은 +0.01%p로 정확히 0). 표본이 작아 근거가 못 되지만,
#    서울 표본이 50건 넘게 모이면 **거리 비중 하나만** 다시 재볼 만하다.
SIMILARITY_WEIGHTS = {"distance": 0.35, "area": 0.30, "floor": 0.20, "build_year": 0.15}
...
def similarity_score(distance_m: float, radius_m: float,
    ...
    `SIMILARITY_WEIGHTS`로 뒀다(72-9절에서 올렸다가 72-15절 실측으로 되돌린 값 —
...
# 70절 — 비교거래가 얇으면 **높게 부른다**(방향을 아는 유일한 경고).
# 합친 125건에서 비교거래 수 ↔ 편향 스피어만 ρ=−0.223(순열검정 p=0.011),
# 임계값을 3~12 어디로 끊어도 **전부 같은 방향**이고 5~9에서 유의하다.
# ⚠️ 계단이 아니라 완만한 관계라 경계값 자체에 큰 의미는 없다 — 경고가 너무
#    자주 뜨지 않도록(전체의 19%) 가장 좁게 잡은 값이다.
# ⚠️ **가격은 안 바꾼다.** 66절이 축소(shrinkage)를 역보정하지 않기로 한 것과
...
# ⛔ **지역(서울/경기)도 위험요인으로 넣지 않는다** — 72-24절에서 측정하고 기각했다.
#    서울이 더 틀리는 것(MAPE +5.1%p)은 사실이지만 **이미 위 세 요인이 잡아낸다**
#    (서울은 구축 44%·소형 51%·괴리 60%로 애초에 위험요인 개수가 높다). 넣으면
#    같은 것을 두 번 세서 **갈라내는 폭이 13.5 → 11.6%p로 오히려 나빠진다.**
#    그리고 표본에 서울·경기밖에 없어 **"비서울"이 부산·대구로 일반화될 근거가 0이다.**
...
def compute_estimate_warnings(filtered: list[dict],
    ...
    if model_divergence_pct is not None and model_divergence_pct >= ESTIMATE_WARN_DIVERGENCE_PCT:
        warnings.append({
            "key": "divergence",
            # 72-33절 — 예전 문구("총액 기준과 ㎡당가 기준 추정이 N% 차이")는
            #   업계 용어 그대로라 사용자가 "무슨 말인지 나도 이해 못 하겠다"고 했다.
            #   **무엇이 문제인지**(비교한 집 크기가 안 맞는다)를 먼저 말하고,
            #   두 계산 방식은 쉬운 말로 풀어서 뒤에 붙인다.
            "label": "비교한 집들의 크기가 이 집과 잘 안 맞아요",
            "detail": ("값을 매기는 방법이 두 가지예요 — ① 비슷한 집이 "
                       "통째로 얼마에 팔렸는지 보는 것, ② 1㎡에 얼마였는지 구해서 "
                       "이 집 넓이에 곱하는 것. 보통은 두 답이 거의 같은데 지금은 "
                       f"{model_divergence_pct:.1f}%나 차이 납니다. 비교한 집들이 "
                       "이 집보다 유난히 크거나 작을 때 이렇게 갈려요."),
            "advice": ("첫 화면 '상세 옵션'에서 면적 허용범위(기본 ±15%)를 좁혀 "
                       "보세요 — 넓이가 더 비슷한 집만 비교하게 됩니다."),
        })

    # ⑥ 70절 — 비교거래가 얇다. ⚠️ **위 ①과 성격이 다르다**: 괴리율은 "얼마나
    #    틀릴지"(크기)를 가르지만 방향은 모르고(편향 +0.2% [−5.3, +5.8]),
    ...
    if 0 < n_comp < ESTIMATE_WARN_THIN_SAMPLE and _is_old:
        warnings.append({
            "key": "thin_sample",
            "label": "오래된 집인데 비교할 거래가 너무 적어요",
            "detail": (f"{_year}년에 지어진 집인데 조건이 맞는 실거래를 "
                       f"{n_comp}건밖에 못 찾았어요. 오래된 집은 같은 해에 지어졌어도 "
                       "수리를 했는지, 관리가 잘 됐는지, 땅 지분이 얼마인지에 따라 "
                       "값이 많이 갈립니다. 비교할 거래까지 적으면 그 차이를 못 보고 "
                       "동네 평균 쪽으로 끌려가요. 과거 실거래로 다시 맞혀 보니 "
                       "이런 경우에만 평균 20%쯤 높게 불렀습니다 "
                       "(해당 사례가 10건뿐이라 참고만 하세요. 지은 지 얼마 안 된 "
                       "집은 거래가 적어도 이런 쏠림이 없었어요)."),
            "advice": ("첫 화면 '상세 옵션'에서 반경이나 면적 허용범위를 넓혀 "
                       "비교할 거래를 더 모아 보세요. 그대로 쓰신다면 이 값을 "
                       "'이보다 비싸게 팔리긴 어렵다'는 상한으로 보시는 게 안전합니다."),
        })
...
def print_estimate_warnings(warnings: list[dict]):
    ...
    print("  비슷한 신호가 뜬 추정치는 실제로 더 많이 빗나갔습니다"
          " (과거 실거래 125건: 신호 없음 10.8% · 있음 14.6%).")
    for w in warnings:
        print(f"  · {w['label']}")
        print(f"      {w['detail']}")
        print(f"      → {w['advice']}")
